...
def calc_search_endtime(input_time, search_time, post_time):
...
def calc_next_posttime(input_time, post_time):
...
def parse_date(logger, date_iso, date_name, search_time, post_time):
...
def date_setup(self):
    ...
    if start_date is None:
        ...
        with open(self.paths["prevsearch"], "r", encoding="utf-8") as f:
            ...
            start_time, start_date = parse_date(self.logger, next(f), "start-date", self.search_time, self.post_time)

    # Print some information. Useful to do it before error checks so that all information is visible
    self.logger.info("Searching from {:}/{:}/{:} 19:00 UTC to {:}/{:}/{:} 19:00 UTC".format(start_date.year, start_date.month, start_date.day, end_date.year, end_date.month, end_date.day))

    return start_date, end_date

def date_errorcheck(self):
