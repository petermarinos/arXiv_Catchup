...
class ArxivClient:
    ...
    def __init__(
        ...
        self.ssl_dict = {"ssl_context": None, "ssl_preverr": False}
    ...
    def get_search_info(self, arxiv_const: ArxivConst, xml_path: pathlib.Path) -> None:
        ...
        if not os.path.exists(xml_path):
            ...
            # Perform the query
            xml_data, self.ssl_dict = arxiv_query(
                self.logger, self.ssl_dict, self.url, 0, 1
            )
        ...
        if max_num_temp is None:
            self.logger.critical(
                "arXiv data did not include a number of papers. It is corrupted (returned None).\n"
            )
            raise
        max_num_str = max_num_temp.text
        if max_num_str is None:
            self.logger.critical(
                "arXiv data for the number of papers is corrupted (returned None).\n"
            )
            raise
    ...
    def arxiv_error_check(
        self, arxiv_const: ArxivConst, xml_path: pathlib.Path
    ) -> None:
        ...
        time_to_search_minutes = (
            self.total_papers
            * arxiv_const.sleep_search
            / (arxiv_const.search_blocksize * 60)
        )
        ...
        if 1 <= time_to_search_minutes < 5:
            ...
            self.logger.warning(
                "There are {:d} papers. The search will take {:.1f} minutes.".format(
                    self.total_papers, time_to_search_minutes
                )
            )
        ...
        # Prompt the user if it is going to take a really long time.
            ...
            user_prompt = (
                input(
                    "There are {:d} papers. The search will take {:.1f} minutes. Continue? [y/N]: ".format(
                        self.total_papers, time_to_search_minutes
                    )
                )
                .strip()
