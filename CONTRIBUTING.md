# Contributing

I am using this project as a way of improving my coding skills, so I would prefer issues suggesting improvements.
However, if there is something small, feel free to contribute!

## Installation

Clone the repo via the command `$ git clone https://github.com/petermarinos/arXiv_Catchup.git`.
Change into the cloned directory via `$ cd ./arXiv_Catchup`, then use the command `$ python -m pip install -e ".[dev]"` to install with the development tools.
To install with the GUI and the dev tools, use the command `$ python -m pip install -e ".[dev,gui]"`.

As with installing the non-dev version, it is recommended to create a new python environment.

Tests can be performed by running `python3 -m pytest tests` from the top directory.

## Development workflow

- Stick to Australian-English spelling conventions.
- Follow PEP 8 naming conventions (e.g. snake_case for functions, CapWords for classes).
- Add type hints on everything, check with PyLance and mypy.
- Use the black formatter and the PyLint linter

## Reporting issues

When reporting a bug, please include the catchup.log flie.